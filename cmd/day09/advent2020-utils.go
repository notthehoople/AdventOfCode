...
func catchUserInput() (string, byte, bool, bool, int) {
	var debug bool
	var test bool
	var preamble int
	...
	flag.IntVar(&preamble, "pre", 5, "Preamble")

	flag.Parse()

	switch *execPartPtr {
	case "a":
		return *filenamePtr, 'a', debug, test, preamble
	case "b":
		return *filenamePtr, 'b', debug, test, preamble

	default:
		return *filenamePtr, 'z', debug, test, preamble
	}
}
